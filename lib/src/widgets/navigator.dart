...
abstract class Route<T> {
  ...
  bool get isCurrent {
    ...
    final _RouteEntry? currentRouteEntry =
        _navigator!._history.cast<_RouteEntry?>().lastWhere(
              (_RouteEntry? e) =>
                  e != null && _RouteEntry.isPresentPredicate(e),
              orElse: () => null,
            );
  ...
  bool get isActive {
    if (_navigator == null) return false;
    return _navigator!._history
            .cast<_RouteEntry?>()
            .firstWhere(
              (_RouteEntry? e) =>
                  e != null && _RouteEntry.isRoutePredicate(this)(e),
              orElse: () => null,
            )
            ?.isPresent ==
        true;
...
class RouteSettings {
  ...
  @override
  String toString() =>
      '${objectRuntimeType(this, 'RouteSettings')}("$name", $arguments)';
}
...
class _RouteEntry extends RouteTransitionRecord {
  _RouteEntry(
    ...
    this.restorationInformation,
        ...
        assert(initialState == _RouteLifecycle.staging ||
  ...
  void handlePush(
    ...
    if (previousState == _RouteLifecycle.replace ||
      ...
      navigator._observedRouteAdditions
          .add(_NavigatorReplaceObservation(route, previousPresent));
    } else {
  ...
  void handlePop(
    ...
    navigator._observedRouteDeletions
        .add(_NavigatorPopObservation(route, previousPresent));
  }
  ...
  void handleRemoval(
    ...
    if (_reportRemovalToObserver) {
      navigator._observedRouteDeletions
          .add(_NavigatorRemoveObservation(route, previousPresent));
    }
  }
  ...
  static final _RouteEntryPredicate isPresentPredicate =
      (_RouteEntry entry) => entry.isPresent;
  static final _RouteEntryPredicate suitableForTransitionAnimationPredicate =
      (_RouteEntry entry) => entry.suitableForTransitionAnimation;
  static final _RouteEntryPredicate willBePresentPredicate =
      (_RouteEntry entry) => entry.willBePresent;
...
class NavigatorState extends State<Navigator>
          ...
          if (newHeroController.navigator != null) {
            ...
            ServicesBinding.instance!
              ...
              if (_heroControllerFromScope == newHeroController) {
                ...
                assert(previousOwner._heroControllerFromScope !=
                    newHeroController);
              }
            });
          }
    ...
    while (oldEntriesBottom <= oldEntriesTop) {
      ...
      if (!potentialEntryToRemove.hasPage) {
        ...
        if (previousOldPageRouteEntry!.isWaitingForExitingDecision &&
            potentialEntryToRemove.isPresent)
    ...
    while ((oldEntriesBottom <= oldEntriesTop) &&
      ...
      if (!oldEntry.hasPage) {
        ...
        final List<_RouteEntry> pagelessRoutes = pageRouteToPagelessRoutes
            .putIfAbsent(previousOldPageRouteEntry, () => <_RouteEntry>[]);
        pagelessRoutes.add(oldEntry);
        continue;
      }
      previousOldPageRouteEntry = oldEntry;
    ...
    if (widget.reportsRouteUpdateToEngine) {
      ...
      if (routeName != _lastAnnouncedRouteName) {
        SystemNavigator.routeUpdated(
            routeName: routeName, previousRouteName: _lastAnnouncedRouteName);
  ...
  Future<T?> pushReplacement<T extends Object?, TO extends Object?>(
    ...
    _pushReplacementEntry(
        _RouteEntry(newRoute, initialState: _RouteLifecycle.pushReplace),
        result);
  ...
  void replace<T extends Object?>(
    ...
    assert(newRoute != null);
    _replaceEntry(
        _RouteEntry(newRoute, initialState: _RouteLifecycle.replace), oldRoute);
  }
  ...
  void _replaceEntry(_RouteEntry entry, Route<dynamic> oldRoute) {
    assert(!_debugLocked);
    if (oldRoute == entry.route) return;
    assert(() {
      _debugLocked = true;
      return true;
    }());
    ...
    if (wasCurrent) _afterNavigation(entry.route);
  }
  ...
  void replaceRouteBelow<T extends Object?>(
    ...
    _replaceEntryBelow(
        _RouteEntry(newRoute, initialState: _RouteLifecycle.replace),
        anchorRoute);
  }
  ...
  void popUntil(RoutePredicate predicate) {
    ...
    while (candidate != null) {
      if (predicate(candidate.route)) return;
      pop();
...
class _NamedRestorationInformation extends _RestorationInformation {
  ...
  @override
  Route<dynamic> createRoute(NavigatorState navigator) {
    final Route<dynamic> route = navigator._routeNamed<dynamic>(name,
        arguments: arguments, allowNull: false)!;
    assert(route != null);
    return route;
  }
}
  ...
  void update(List<_RouteEntry> history) {
    ...
    for (final _RouteEntry entry in history) {
      ...
      restorationEnabled = restorationEnabled &&
          entry.restorationInformation?.isRestorable == true;
      entry.restorationEnabled = restorationEnabled;
      if (restorationEnabled) {
        assert(entry.restorationId != null);
...
class RestorableRouteFuture<T> extends RestorableProperty<String?> {
  ...
  RestorableRouteFuture({
    ...
    this.onComplete,
  })  : assert(onPresent != null),
        assert(navigatorFinder != null);
