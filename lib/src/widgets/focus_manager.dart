...
import 'package:flute/fake_async.dart' show scheduleMicrotask;
...
typedef FocusOnKeyCallback = dynamic Function(
...
class FocusAttachment {
  ...
  void detach() {
    assert(_node != null);
    assert(_focusDebug('Detaching node:', <String>[
      _node.toString(),
      'With enclosing scope ${_node.enclosingScope}'
    ]));
    if (isAttached) {
...
class FocusNode with DiagnosticableTreeMixin, ChangeNotifier {
  ...
  Iterable<FocusNode> get traversalDescendants => descendants
  ...
  void unfocus({
    ...
    assert(_focusDebug('Unfocused node:', <String>[
